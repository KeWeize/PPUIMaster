 ...
 * @date:   2019/11/11
 */
open class PUIRoundLinearLayout : PUIAlphaLinearLayout {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val ta =
            context.obtainStyledAttributes(attrs, R.styleable.PUIRoundLinearLayout, defStyleAttr, 0)
        val mBuilder = PUIRoundDrawable.Builder()

        mBuilder.primaryColor =
            ta.getColor(R.styleable.PUIRoundLinearLayout_pui_primaryColor, Color.TRANSPARENT)
        mBuilder.secondaryColor =
            ta.getColor(R.styleable.PUIRoundLinearLayout_pui_secondaryColor, Color.TRANSPARENT)
        mBuilder.borderColor =
            ta.getColor(R.styleable.PUIRoundLinearLayout_pui_borderColor, Color.TRANSPARENT)
        mBuilder.borderWidth =
            ta.getDimensionPixelSize(R.styleable.PUIRoundLinearLayout_pui_borderWidth, 0)
        mDisableColor =
            ta.getColor(R.styleable.PUIRoundLinearLayout_pui_disableColor, Color.TRANSPARENT)
        mPressedAlpha = ta.getFloat(R.styleable.PUIRoundLinearLayout_pui_pressedAlpha, 1f)
        mOrientationFlag = ta.getInt(R.styleable.PUIRoundLinearLayout_pui_gradientOrientation, 0)

        mBuilder.radius = ta.getDimensionPixelSize(R.styleable.PUIRoundLinearLayout_pui_radius, 0)
        mBuilder.radiusTopLeft =
            ta.getDimensionPixelSize(R.styleable.PUIRoundLinearLayout_pui_radiusTopLeft, 0)
        mBuilder.radiusTopRight =
            ta.getDimensionPixelSize(R.styleable.PUIRoundLinearLayout_pui_radiusTopRight, 0)
        mBuilder.radiusBottomLeft =
            ta.getDimensionPixelSize(R.styleable.PUIRoundLinearLayout_pui_radiusBottomLeft, 0)
        mBuilder.radiusBottomRight =
            ta.getDimensionPixelSize(R.styleable.PUIRoundLinearLayout_pui_radiusBottomRight, 0)
        mBuilder.isRadiusAdjustBounds =
            ta.getBoolean(R.styleable.PUIRoundLinearLayout_pui_isRadiusAdjustBounds, true)
