 ...
 * @date:   2019/11/6
 */
open class PUIRoundFrameLayout : FrameLayout {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.PUIRoundFrameLayout, defStyleAttr, 0)
        val count = ta.indexCount
        val mBuilder = PUIRoundDrawable.Builder()
        var orientationFlag = 0
        for (i in 0 until count) {
            when (val index = ta.getIndex(i)) {
                R.styleable.PUIRoundFrameLayout_pui_backgroundColor ->
                    mBuilder.colorBg = ta.getColorStateList(index)
                R.styleable.PUIRoundFrameLayout_pui_borderColor ->
                    mBuilder.colorBorder = ta.getColorStateList(index)
                R.styleable.PUIRoundFrameLayout_pui_borderWidth ->
                    mBuilder.borderWidth = ta.getDimensionPixelSize(index, 0)
                R.styleable.PUIRoundFrameLayout_pui_isRadiusAdjustBounds ->
                    mBuilder.isRadiusAdjustBounds = ta.getBoolean(index, true)
                R.styleable.PUIRoundFrameLayout_pui_radius ->
                    mBuilder.mRadius = ta.getDimensionPixelSize(index, 0)
                R.styleable.PUIRoundFrameLayout_pui_radiusTopLeft ->
                    mBuilder.mRadiusTopLeft = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundFrameLayout_pui_radiusTopRight ->
                    mBuilder.mRadiusTopRight = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundFrameLayout_pui_radiusBottomLeft ->
                    mBuilder.mRadiusBottomLeft = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundFrameLayout_pui_radiusBottomRight ->
                    mBuilder.mRadiusBottomRight = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundFrameLayout_pui_gradientOrientation ->
                    orientationFlag =
                        ta.getInt(R.styleable.PUIRoundFrameLayout_pui_gradientOrientation, 0)
                R.styleable.PUIRoundFrameLayout_pui_gradientStartColor ->
                    mBuilder.mGradientColorStart = ta.getColor(
                        R.styleable.PUIRoundFrameLayout_pui_gradientStartColor,
                        Color.TRANSPARENT
                    )
                R.styleable.PUIRoundFrameLayout_pui_gradientEndColor ->
