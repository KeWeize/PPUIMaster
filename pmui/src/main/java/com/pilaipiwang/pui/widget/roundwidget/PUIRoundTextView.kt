    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        for (i in 0 until count) {
            when (val index = ta.getIndex(i)) {
                R.styleable.PUIRoundTextView_pui_backgroundColor ->
                    mBuilder.colorBg = ta.getColorStateList(index)
                R.styleable.PUIRoundTextView_pui_borderColor ->
                    mBuilder.colorBorder = ta.getColorStateList(index)
                R.styleable.PUIRoundTextView_pui_borderWidth ->
                    mBuilder.borderWidth = ta.getDimensionPixelSize(index, 0)
                R.styleable.PUIRoundTextView_pui_isRadiusAdjustBounds ->
                    mBuilder.isRadiusAdjustBounds = ta.getBoolean(index, true)
                R.styleable.PUIRoundTextView_pui_radius ->
                    mBuilder.mRadius = ta.getDimensionPixelSize(index, 0)
                R.styleable.PUIRoundTextView_pui_radiusTopLeft ->
                    mBuilder.mRadiusTopLeft = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundTextView_pui_radiusTopRight ->
                    mBuilder.mRadiusTopRight = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundTextView_pui_radiusBottomLeft ->
                    mBuilder.mRadiusBottomLeft = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundTextView_pui_radiusBottomRight ->
                    mBuilder.mRadiusBottomRight = ta.getDimensionPixelSize(index, 0).toFloat()
                R.styleable.PUIRoundTextView_pui_gradientOrientation ->
                    orientationFlag =
                        ta.getInt(R.styleable.PUIRoundTextView_pui_gradientOrientation, 0)
                R.styleable.PUIRoundTextView_pui_gradientStartColor ->
                    mBuilder.mGradientColorStart = ta.getColor(
                        R.styleable.PUIRoundTextView_pui_gradientStartColor,
                        Color.TRANSPARENT
                    )
                R.styleable.PUIRoundTextView_pui_gradientEndColor ->
